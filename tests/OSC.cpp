...
OSCMessage::OSCMessage(char* buf, int length) {
  ...
  // Check if there's a type tag string (starts with comma)
  int parameterCount = 0;
  if(buf[i] == ',') {
    // Standard OSC with type tags
    i++; // Skip the comma
    ...
    i = i + (4 - i%4); // Skip padding after type tags
  } else {
    // Non-standard OSC without type tags - assume single float parameter
    // This handles some implementations that send simplified OSC messages
    paramTypes[0] = 'f';
    parameterCount = 1;
  }
...
String OSCMessage::getString(byte pos){
  ...
  memcpy(strBuf, (char*)(uintptr_t)blobs[pos].s[0], blobs[pos].s[1]);
  return String(strBuf);
}

byte OSCMessage::getParameterCount(){
  return parameterCount;
}

char OSCMessage::getParameterType(byte pos){
